...
<body>
    <div class="container">
        ...
        <div class="right-panel">
            ...
            <div class="metric-card">
                <div class="metric-header">
                    <span class="metric-name">🦴 关节角度（区分左右侧）</span>
                ...
                <div class="metric-detail">
                    ⚠️ <strong>得分依据：</strong>分别分析左侧和右侧关节活动度<br>
                    • 膝关节（左）屈曲：133° → 理想 140-160°（⚠️ 不足）<br>
                    • 膝关节（右）屈曲：137° → 理想 140-160°（⚠️ 略不足）<br>
                    • 髋关节（左）伸展：147° → 理想 150-170°（⚠️ 略不足）<br>
                    • 髋关节（右）伸展：149° → 理想 150-170°（⚠️ 接近达标）<br>
                    • 肩关节（左）外展：177° → 理想 170-180°（✅ 正常）<br>
                    • 肩关节（右）外展：179° → 理想 170-180°（✅ 正常）<br><br>
                    💡 <strong>主要问题：</strong>膝关节屈曲角度双侧均不足，右侧优于左侧 4°，提示左侧膝关节灵活性需重点改善
                </div>
                <!-- 关节角度详细图表（区分左右） -->
                <div class="detail-section">
                    <h4>📊 主要关节角度变化（左vs右）</h4>
            ...
            <div class="metric-card">
                <div class="metric-header">
                    <span class="metric-name">💪 肌肉激活度（区分左右侧）</span>
                ...
                <div class="metric-detail">
                    ⚠️ <strong>得分依据：</strong>分别分析左侧和右侧主要推进肌群的激活程度<br>
                    • 臀大肌（左）：82% → 理想 ≥80%（✅ 充分）<br>
                    • 臀大肌（右）：88% → 理想 ≥80%（✅ 充分）<br>
                    • 腘绳肌（左）：60% → 理想 ≥75%（⚠️ 不足）<br>
                    • 腘绳肌（右）：64% → 理想 ≥75%（⚠️ 不足）<br>
                    • 股四头肌（左）：76% → 理想 ≥75%（✅ 正常）<br>
                    • 股四头肌（右）：80% → 理想 ≥75%（✅ 正常）<br><br>
                    💡 <strong>主要问题：</strong>腘绳肌双侧激活不足，左侧尤甚（仅60%），可能导致膝关节稳定性下降
                </div>
                <!-- 肌肉激活详细图表 -->
                <div class="detail-section">
                    <h4>📊 主要肌肉激活度对比（左vs右）</h4>
                    <div class="chart-wrapper" style="height: 200px;">
    ...
    <script>
        ...
        window.addEventListener('load', function() {
            ...
            new Chart(jointCtx, {
                type: 'bar',
                data: {
                    labels: ['膝关节(左)', '膝关节(右)', '髋关节(左)', '髋关节(右)', '肩关节(左)', '肩关节(右)', '踝关节(左)', '踝关节(右)'],
                    datasets: [{
                        label: '当前角度',
                        data: [133, 137, 147, 149, 177, 179, 14, 16],
                        backgroundColor: ['#667eea', '#764ba2', '#667eea', '#764ba2', '#667eea', '#764ba2', '#667eea', '#764ba2'],
                        borderColor: '#667eea',
                        borderWidth: 1
                    }, {
                        label: '理想最小值',
                        data: [140, 140, 150, 150, 170, 170, 20, 20],
            ...
            new Chart(muscleCtx, {
                type: 'bar',
                data: {
                    labels: ['臀大肌(左)', '臀大肌(右)', '腘绳肌(左)', '腘绳肌(右)', '股四头肌(左)', '股四头肌(右)', '腓肠肌(左)', '腓肠肌(右)'],
                    datasets: [{
                        label: '激活度(%)',
                        data: [82, 88, 60, 64, 76, 80, 70, 74],
                        backgroundColor: ['#667eea', '#764ba2', '#667eea', '#764ba2', '#667eea', '#764ba2', '#667eea', '#764ba2'],
                        borderColor: '#667eea',
                        borderWidth: 1
                    }, {
                        label: '理想值(%)',
                        data: [80, 80, 75, 75, 75, 75, 70, 70],
                        type: 'line',
                        fill: false,
                        borderColor: '#10b981',
                        borderWidth: 2,
    ...
    # 构建客户信息卡片HTML
    client_info_html = ''
    has_client_info = age or gender or height or weight
    
    if has_client_info:
        client_info_html = '<div class="client-info-card"><h3>📋 客户基础信息</h3><div class="client-info-grid">'
        
        if age:
            client_info_html += f'<div class="info-item"><div class="info-value">{age}岁</div><div class="info-label">年龄</div></div>'
        else:
            client_info_html += '<div class="info-item"><div class="info-value">-</div><div class="info-label">年龄</div></div>'
            
        if gender:
            gender_text = '男' if gender == 'male' else '女'
            client_info_html += f'<div class="info-item"><div class="info-value">{gender_text}</div><div class="info-label">性别</div></div>'
        else:
            client_info_html += '<div class="info-item"><div class="info-value">-</div><div class="info-label">性别</div></div>'
            
        if height:
            client_info_html += f'<div class="info-item"><div class="info-value">{height}cm</div><div class="info-label">身高</div></div>'
        else:
            client_info_html += '<div class="info-item"><div class="info-value">-</div><div class="info-label">身高</div></div>'
            
        if weight:
            client_info_html += f'<div class="info-item"><div class="info-value">{weight}kg</div><div class="info-label">体重</div></div>'
        else:
            client_info_html += '<div class="info-item"><div class="info-value">-</div><div class="info-label">体重</div></div>'
            
        client_info_html += '</div></div>'
        ...
        if age:
            age_int = int(age)
            if age_int < 30:
                analysis_notes.append(f'年龄 {age}岁：处于运动能力巅峰期，关节灵活性好，恢复快')
            elif age_int < 50:
                analysis_notes.append(f'年龄 {age}岁：肌肉力量稳定，需注意关节保养和柔韧性维持')
            else:
                analysis_notes.append(f'年龄 {age}岁：关节灵活性自然下降，需加强热身和恢复性训练')
        ...
        if height and weight:
            ...
            if bmi < 18.5:
                analysis_notes.append(f'BMI {bmi:.1f}：偏瘦，需注意肌肉量不足可能影响关节稳定性')
            elif bmi < 24:
                analysis_notes.append(f'BMI {bmi:.1f}：正常范围，身体条件良好')
            elif bmi < 28:
                analysis_notes.append(f'BMI {bmi:.1f}：偏重，额外负荷可能影响关节压力和动作流畅度')
            else:
                analysis_notes.append(f'BMI {bmi:.1f}：超重，建议结合低冲击运动（如游泳）减少关节负担')
        
        if analysis_notes:
            client_info_html += '<div class="detail-section" style="margin-top: 12px;"><h4>📊 基于您的基础信息的分析参考</h4>'
            for note in analysis_notes:
                client_info_html += f'<p style="margin-bottom: 6px;">• {note}</p>'
            client_info_html += '</div>'
    else:
        client_info_html = '<div class="client-info-card"><div class="info-empty">未填写基础信息，<a href="/" style="color: #667eea;">点击返回填写</a>可获得更精准的分析</div></div>'
    
    # 替换占位符
    html_output = html_template.replace('FILENAME_PLACEHOLDER', filename)
    html_output = html_output.replace('CLIENT_INFO_PLACEHOLDER', client_info_html)
    
    return html_output

if __name__ == '__main__':
    print('=' * 70)
    print('  AFP 运动康复AI分析系统 v22.0')
    print('  ✅ 客户基础信息版 - 结合真实数据，分析更精准')
    print('=' * 70)
    print()
    ...
    print('  新增功能:')
    print('    ✅🔥 1.首页增加客户基础信息表单（年龄、性别、身高、体重）')
    print('    ✅🔥 2.关节角度明确区分左侧/右侧（数据+图表+表格）')
    print('    ✅🔥 3.肌肉激活度明确区分左侧/右侧')
    print('    ✅🔥 4.统一色彩方案（主色蓝紫 #667eea/#764ba2）')
    print('    ✅🔥 5.结合客户真实数据给出分析参考（不夸大）')
    print('    ✅   6.客户信息卡片展示在分析页顶部')
    print()
    print('=' * 70)
    print()
