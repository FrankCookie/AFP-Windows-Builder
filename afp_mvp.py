...
def generate_simulation_data(age, gender, height, weight):
    ...
    if height and weight:
        ...
        if bmi > 25:
            joint_pressure = "偏高"
            joint_detail = f"BMI {bmi:.1f}，关节压力增加"
        elif bmi > 22:
            joint_pressure = "中等"
            joint_detail = f"BMI {bmi:.1f}，关节压力适中"
        else:
            joint_pressure = "正常"
            joint_detail = f"BMI {bmi:.1f}，关节压力正常"
    else:
        joint_pressure = "未评估"
        joint_detail = "请输入身高体重"
    ...
    # 5. 重心偏移数据
    center_offset_data = [
        {"phase": "左单脚支撑", "value": round(random.uniform(-8, -3), 1), "meaning": "左侧偏移"},
        {"phase": "右单脚支撑", "value": round(random.uniform(3, 8), 1), "meaning": "右侧偏移"},
        {"phase": "双脚支撑", "value": round(random.uniform(-2, 2), 1), "meaning": "接近中线"},
        {"phase": "左脚蹬伸", "value": round(random.uniform(-6, -2), 1), "meaning": "左侧偏移"},
    ]
    
    # 6. 动作对称性数据
    symmetry_data = [
        {"metric": "左-右地面反作用力比", "left": round(0.45 + random.uniform(-0.05, 0.05), 3), 
         "right": round(0.45 + random.uniform(-0.05, 0.05), 3), "unit": ""},
        {"metric": "左-右力矩生成比", "left": round(0.48 + random.uniform(-0.08, 0.08), 3), 
         "right": round(0.48 + random.uniform(-0.08, 0.08), 3), "unit": ""},
        {"metric": "左-右力链传递效率", "left": round(0.85 + random.uniform(-0.10, 0.10), 3), 
         "right": round(0.85 + random.uniform(-0.10, 0.10), 3), "unit": ""},
    ]
    ...
    return {
        ...
        'left_right_diff': round(left_right_diff, 3),
        ...
        <div class="header">
            ...
            <div class="description">
                ...
                <p style="margin-top: 15px; padding: 15px; background: #1a1a1a; border-left: 4px solid #4CAF50; border-radius: 4px;">
                    ...
                    本系统为MVP模拟演示版本。上传视频后，系统会根据客户信息（年龄、性别、身高、体重）生成个性化的模拟分析数据。<br>
                    所有数据均为智能模拟生成，用于展示AFP分析系统的界面和功能。正式版本将接入真实的视频分析算法。
                </p>
            </div>
        </div>
    ...
    <div class="container">
        ...
        <div class="client-info">
            ...
            <div class="info-card">
                ...
                <div class="value">{{ gender_text }}</div>
            </div>
            <div class="info-card">
                <div class="label">身高/体重</div>
                <div class="value">{{ height }}cm/{{ weight }}kg</div>
            </div>
            <div class="info-card">
                <div class="label">关节压力</div>
                <div class="value" style="font-size: 18px;">{{ joint_pressure }}</div>
            </div>
        </div>
        ...
        <div class="scores">
            <div class="score-card">
                <div class="score-label">动作对称性</div>
                <div class="score-value" id="score-symmetry">{{ scores.symmetry }}</div>
                <div class="score-unit">/ 100</div>
            </div>
            <div class="score-card">
                <div class="score-label">力量输出</div>
                <div class="score-value" id="score-force">{{ scores.force }}</div>
                <div class="score-unit">/ 100</div>
            </div>
            <div class="score-card">
                <div class="score-label">全身协调性</div>
                <div class="score-value" id="score-coordination">{{ scores.coordination }}</div>
                <div class="score-unit">/ 100</div>
            </div>
        </div>
        ...
        <div class="data-table">
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ item.metric }}</td>
                        <td>{{ "%.3f"|format(item.left) }}</td>
                        <td>{{ "%.3f"|format(item.right) }}</td>
                    </tr>
                    {% endfor %}
                </tbody>
            </table>
        ...
        <div class="data-table">
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ item.phase }}</td>
                        <td>{{ "%.2f"|format(item.force) }}</td>
                        <td>{{ "%.3f"|format(item.efficiency) }}</td>
                    </tr>
                    {% endfor %}
                </tbody>
            </table>
    ...
    <script>
            ...
            data: {
                ...
                datasets: [{
                    ...
                    data: [
                        ...
                        {{ (scores.symmetry + scores.force + scores.coordination) / 3 }}
                    ],
        ...
        new Chart(offsetCtx, {
            ...
            data: {
                ...
                datasets: [{
                    label: '重心偏移 (cm)',
                    data: [{% for item in center_offset_data %}{{ item.value }},{% endfor %}],
                    backgroundColor: [
                        'rgba(255, 99, 132, 0.6)',
                        'rgba(54, 162, 235, 0.6)',
                        'rgba(75, 192, 192, 0.6)',
...
def upload():
    ...
    try:
        ...
        template_data = {
            ...
            'height': height,
            'weight': weight,
            'joint_pressure': simulation_data['joint_detail'],
        ...
        return render_template_string(REPORT_TEMPLATE, **template_data)
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    print("⚠️  模拟演示版说明:")
    print("   1. 本系统生成的数据为智能模拟数据")
    print("   2. 根据客户信息（年龄、性别、身高、体重）个性化生成")
    print("   3. 正式版本将接入真实视频分析算法")
    print("=" * 60)
